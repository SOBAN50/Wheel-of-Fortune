...
NoOfPlayers = input("Enter number of players playing this game(must be 2 or more) : ")

if(NoOfPlayers.isdigit()):
    while(int(NoOfPlayers)<=1):
        NoOfPlayers = int(input("Number of players must be 2 or more. Enter again : "))
else:
    while(not NoOfPlayers.isdigit()):
        NoOfPlayers = input("Please enter a number : ")
    NoOfPlayers = int(NoOfPlayers)


for i in range(int(NoOfPlayers)):
    inp = input("Player "+ str(i+1)+ " Name : ")
    nam = Player(inp)
    PlayersList.append(nam)
print('\n\n')
    ...
    if(Prize == "Bankrupt"):#------------------------------------Bankrupt
        ...
        if(Choice == "1"):
            ...
            if(input_letter == "pass"):
                ...
                if(len(occurances_array) == 0):PlayersList[Turn].add_cash(int(Prize))
                else:PlayersList[Turn].add_cash(int(Prize)*len(occurances_array))
